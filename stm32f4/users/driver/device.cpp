...
usart::usart(uint8_t x, uint32_t rate) : x(x) {
    ...
    nvic_cfg.NVIC_IRQChannelPreemptionPriority = 0x4;
    nvic_cfg.NVIC_IRQChannelSubPriority = 0x4;
...
time::time(uint8_t x) : x(x) {
    ...
    nvic_cfg.NVIC_IRQChannelPreemptionPriority = 0x04;
    nvic_cfg.NVIC_IRQChannelSubPriority = 0x04;
    nvic_cfg.NVIC_IRQChannelCmd = ENABLE;
...
//    SPIX[x - 1].res_fun(SPIX[x - 1].rcc, DISABLE);
    ...
    cfg.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_32;
