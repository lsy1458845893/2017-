...
static void tt_opt_delay(volatile u8 i) {
    while (i--);
}

void LCD_DEV::black_light(uint8_t duty) {

    LCD_WR_REG(0xBE);    //配置PWM输出
    LCD_WR_DATA(0x05);    //1设置PWM频率
    LCD_WR_DATA(duty * 2.55);//2设置PWM占空比
...
void LCD_DEV::LCD_WR_REG(vu16 regval) {

    regval = regval;        //使用-O2优化的时候,必须插入的延时
    LCD->LCD_REG = regval;//写入要写的寄存器序号
}

void LCD_DEV::LCD_WR_DATA(vu16 data) {

    data = data;            //使用-O2优化的时候,必须插入的延时
    LCD->LCD_RAM = data;
}

u16 LCD_DEV::LCD_RD_DATA(void) {
    vu16 ram;            //防止被优化
    ram = LCD->LCD_RAM;
    return ram;
}

void LCD_DEV::on(void) {
...
void LCD_DEV::cursor(u16 x0, u16 y0) {

    u16 x, y;
    x = y0;
//    y = lcddev.height - 1 - x0;
    y = x0;
...
void LCD_DEV::write(u16 c) {
    LCD->LCD_RAM = c;
}

u16 LCD_DEV::read_dot(u16 x0, u16 y0) {
...
//    y = lcddev.height - 1 - x0;
    ...
    if (lcddev.id == 0X9320)tt_opt_delay(2);                //FOR 9320,延时2us
...
void LCD_DEV::dot(u16 x0, u16 y0, u32 c) {
    u16 x, y;
    x = y0;
//    y = lcddev.height - 1 - x0;
    y = x0;
...
void LCD_DEV::LCD_WriteReg(u16 LCD_Reg, u16 LCD_RegValue) {

    LCD->LCD_REG = LCD_Reg;        //写入要写的寄存器序号
    LCD->LCD_RAM = LCD_RegValue;//写入数据
}

u16 LCD_DEV::LCD_ReadReg(u16 LCD_Reg) {
...
void LCD_DEV::LCD_WriteRAM_Prepare(void) {
    LCD->LCD_REG = lcddev.wramcmd;
}

void LCD_DEV::LCD_WriteRAM(u16 RGB_Code) {
    LCD->LCD_RAM = RGB_Code;//写十六位GRAM
}

void LCD_DEV::LCD_Scan_Dir(u8 dir) {
    u16 regval = 0;
    u16 dirreg = 0;
    u16 temp;
...
void LCD_DEV::LCD_Display_Dir(u8 dir) {
    if (dir == 0)            //竖屏
    {
        lcddev.dir = 0;    //竖屏
        lcddev.width = 240;
...
void LCD_DEV::LCD_Set_Window(u16 sx, u16 sy, u16 width, u16 height) {
...
LCD_DEV::LCD_DEV() {
    ...
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA|RCC_AHB1Periph_GPIOB | RCC_AHB1Periph_GPIOD | RCC_AHB1Periph_GPIOE | RCC_AHB1Periph_GPIOF |
                           RCC_AHB1Periph_GPIOG, ENABLE);//使能PD,PE,PF,PG时钟
    RCC_AHB3PeriphClockCmd(RCC_AHB3Periph_FSMC, ENABLE);//使能FSMC时钟


    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;//PB15 推挽输出,控制背光
    ...
    } else if (lcddev.id == 0X6804 || lcddev.id == 0XC505)    //6804/C505速度上不去,得降低
    {
        ...
        FSMC_Bank1E->BWTR[6] &= ~(0XF << 0);//地址建立时间（ADDSET）清零
        FSMC_Bank1E->BWTR[6] &= ~(0XF << 8);//数据保存时间清零
        FSMC_Bank1E->BWTR[6] |= 10 << 0;    //地址建立时间（ADD+SET）为10个HCLK =60ns
