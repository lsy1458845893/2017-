...
void OLED::line_x(uint8_t x, uint8_t y0, uint8_t y1) {
    int y = y0;
    while (y < y1) buffer[y / 8][x] |= 1 << y % 8;
}

void OLED::line_y(uint8_t y, uint8_t x0, uint8_t x1) {
    int x = x0;
    while (x < x1) buffer[y / 8][x] |= 1 << y % 8;
}

void OLED::print(uint8_t line, uint8_t x, char *str) {
    int t = 0;
    while (*str != '\0') {
        for (t = 0; t < 6; ++t) {
...
void OLED::print(uint8_t line, uint8_t y, uint8_t *str) {
    print(line, y, (char *) str);
}

void OLED::setBuffer(uint8_t line, uint8_t y, uint8_t value) {
    buffer[line][y] = value;
}
...
OLED::OLED(GPIO_TypeDef *GPIOx_scl, uint16_t Pin_scl,
           GPIO_TypeDef *GPIOx_sda, uint16_t Pin_sda) :
        GPIOx_SDA(GPIOx_sda), GPIOx_SCL(GPIOx_scl),
        Pin_SDA(Pin_sda), Pin_SCL(Pin_scl) {

    GPIO_INIT_OUT(GPIOx_scl, Pin_scl);
    GPIO_INIT_OUT(GPIOx_sda, Pin_sda);
