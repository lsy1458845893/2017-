...
void ADF4350::send(uint32_t data) {
    int8_t t = 32;
    le.output(0);
    while (t--) {
        sck.output(0);
...
void ADF4350::amp(uint8_t power) {
    send(
            ...
            ADF4350_REG4_OUTPUT_PWR(power) |             //输出功率
            ADF4350_REG4_AUX_OUTPUT_PWR(0x3) |         //辅助输出功率
            ADF4350_REG4
    );
}

void ADF4350::rate(float f) {
    uint32_t INT, FAC;
    ...
    send(
            (ADF4350_REG4_FEEDBACK_FUND |               //反馈选择:从VCO输出到N计数器的反馈,1从VCO获得,0分频器的输出获得
             (uint32_t) (ADF4350_REG4_RF_DIV_SEL(div)) |               //RF分频器选择:选择RF输出分频器的
             ADF4350_REG4_8BIT_BAND_SEL_CLKDIV(0xF0) |   //频段选择时钟分频器值
             //            ADF4350_REG4_VCO_PWRDOWN_EN |              //VCO关断
             ADF4350_REG4_MUTE_TILL_LOCK_EN |           //静音至检测到锁定(MTLD)
             ADF4350_REG4_AUX_OUTPUT_FUND |
    ...
    FAC = (uint32_t) (1250 * (f - INT * 156250) / 156250);
 ...
 * [7,20479]MHz = [[24,65535] + [0,4095] / 1250] * (10MHz / 64)
 *
 * RF Divider是细分VCO频率的输出分频器。 // 1
 *
 * f PFD = REF IN × [(1 + D)/(R × (1 + T))]
 * 10MHz = 100MHz*((1 + 0) / (5*(1 + 1)))
 *
 * REF IN 是参考频率输入;
...
ADF4350::ADF4350(dev::gpio sck, dev::gpio dat, dev::gpio le) :
        sck(sck), dat(dat), le(le) {
