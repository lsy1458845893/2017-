...
const static struct {
    ...
    uint8_t nvic;
        ...
        {SPI1, RCC_APB2PeriphClockCmd, RCC_APB2Periph_SPI1, SPI1_IRQn},
        {SPI2, RCC_APB1PeriphClockCmd, RCC_APB1Periph_SPI2, SPI2_IRQn},
...
void spi::init_slave(void) {
    SPI_InitTypeDef cfg;
    cfg.SPI_Direction = SPI_Direction_Rx | SPI_Direction_Tx;
...
uint16_t spi::exchange(uint16_t dat) {
    SPI_I2S_SendData(SPIX[x - 1].dev, dat);
    while (!SPI_I2S_GetFlagStatus(SPIX[x - 1].dev, SPI_I2S_FLAG_TXE));
    return SPI_I2S_ReceiveData(SPIX[x - 1].dev);
}
...
extern "C" void SPI2_IRQHandler(void) {
    if (SPI_I2S_GetITStatus(SPI2, SPI_I2S_FLAG_RXNE) != RESET) {
...
extern "C" void SPI3_IRQHandler(void) {
    if (SPI_I2S_GetITStatus(SPI3, SPI_I2S_FLAG_RXNE) != RESET) {
        ...
        SPI_I2S_ClearITPendingBit(SPI3, SPI_I2S_FLAG_RXNE);
    }
}
